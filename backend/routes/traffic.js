...
// 쿼리 실행 함수
async function runQuery(query, clickhouse) {
...
router.get("/", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [
      ...
      runQuery(visitorTrendQuery, clickhouse),
      runQuery(hourlyTrafficQuery, clickhouse),
      runQuery(sourceDistQuery, clickhouse),
      runQuery(mediumDistQuery, clickhouse),
      runQuery(campaignDistQuery, clickhouse),
      runQuery(referrerDistQuery, clickhouse),
      runQuery(mainPageNavQuery, clickhouse),
    ]);

    let visitorTrend = visitorTrendResult.map((row) => {
    ...
    let hourlyTraffic = hourlyTrafficResult.map((row) => ({
      hour: row.hour,
    ...
    const sourceDistribution = sourceDistResult.map((row) => ({
      source: row.source || "(없음)",
      visitors: Number(row.visitors),
    }));

    const mediumDistribution = mediumDistResult.map((row) => ({
      medium: row.medium || "(없음)",
      visitors: Number(row.visitors),
    }));

    const campaignDistribution = campaignDistResult.map((row) => ({
      campaign: row.campaign || "(없음)",
      visitors: Number(row.visitors),
    }));

    const referrerDistribution = referrerDistResult.map((row) => ({
      referrer: row.referrer || "(없음)",
      visitors: Number(row.visitors),
    }));

    const mainPageNavigation = mainPageNavResult.map((row, idx) => ({
      name: row.page,
      page: row.page,
