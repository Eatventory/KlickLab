...
export const UserSegmentSummary: React.FC<UserSegmentSummaryProps> = ({ refreshKey, dateRange, data }) => {
  const [summary, setSummary] = useState<string>('');

  useEffect(() => {
    const generateSummary = async () => {
      try {
        // data가 있으면 그걸 사용, 없으면 기존 fetch 로직 사용
        let rows = data;
        if (!rows) {
        ...
        if (rows) rows.forEach((row: any) => {
          // 새로운 API 구조에 맞게 처리
          const userCount = parseInt(row.users ?? row.user_count);
          const gender = row.gender ?? row.segment_value;
          const age = row.age_group ?? row.segment_value;
