...
// 색상 범례 상수
const COLOR_LEGEND = [
  { level: 0, color: '#eff6ff' },
  { level: 1, color: '#dbeafe' },
  { level: 2, color: '#bfdbfe' },
  { level: 3, color: '#93c5fd' },
  { level: 4, color: '#60a5fa' },
...
export const RegionalActiveUsers: React.FC<RegionalActiveUsersProps> = ({ dateRange, onDataSourceUpdate, data, loading: externalLoading }) => {
  ...
  useEffect(() => {
    ...
    if (data) {
      processRegionalData(data);
      setLoading(false);
    }
  }, [data, externalLoading]);

  const processRegionalData = (apiData: any[]) => {
    try {
      // 데이터 소스 정보 업데이트
      ...
      if (Array.isArray(dataArray)) {
        dataArray.forEach((row: any) => {
          ...
          if (city && city !== 'unknown' && users > 0) {
            if (!cityMap[city]) {
              cityMap[city] = 0;
            }
            cityMap[city] += Number(users);
          }
        });
      }
  ...
  const getRegionStyle = useCallback((regionId: string) => {
    ...
    let fillColor = data?.color || '#e5e7eb';
    let filter = 'none';
  ...
  }, [hoveredRegion, selectedRegion, getRegionData]);

  // 이벤트 핸들러들
  const handleRegionHover = useCallback((regionId: string, event: React.MouseEvent) => {
            ...
            >
              ...
              <g>
                <path
                  ...
                  style={getRegionStyle('KR-11')}
                  onMouseEnter={(event) => handleRegionHover('KR-11', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-11')}
                />
                <path
                  id="KR-26"
                  className="land"
                  ...
                  style={getRegionStyle('KR-26')}
                  onMouseEnter={(event) => handleRegionHover('KR-26', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-26')}
                />
                <path
                  id="KR-27"
                  className="land"
                  ...
                  style={getRegionStyle('KR-27')}
                  onMouseEnter={(event) => handleRegionHover('KR-27', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-27')}
                />
                <path
                  id="KR-28"
                  className="land"
                  ...
                  style={getRegionStyle('KR-28')}
                  onMouseEnter={(event) => handleRegionHover('KR-28', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-28')}
                />
                <path
                  id="KR-29"
                  className="land"
                  ...
                  style={getRegionStyle('KR-29')}
                  onMouseEnter={(event) => handleRegionHover('KR-29', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-29')}
                />
                <path
                  id="KR-30"
                  className="land"
                  ...
                  style={getRegionStyle('KR-30')}
                  onMouseEnter={(event) => handleRegionHover('KR-30', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-30')}
                />
                <path
                  id="KR-31"
                  className="land"
                ...
                <path
                  ...
                  style={getRegionStyle('KR-42')}
                  onMouseEnter={(event) => handleRegionHover('KR-42', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-42')}
                />
                <path
                  id="KR-43"
                  className="land"
                  ...
                  style={getRegionStyle('KR-43')}
                  onMouseEnter={(event) => handleRegionHover('KR-43', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-43')}
                />
                <path
                  id="KR-44"
                  className="land"
                  ...
                  style={getRegionStyle('KR-44')}
                  onMouseEnter={(event) => handleRegionHover('KR-44', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-44')}
                />
                <path
                  id="KR-45"
                  className="land"
                  ...
                  style={getRegionStyle('KR-45')}
                  onMouseEnter={(event) => handleRegionHover('KR-45', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-45')}
                />
                <path
                  id="KR-46"
                  className="land"
                ...
                <path
                  ...
                  style={getRegionStyle('KR-48')}
                  onMouseEnter={(event) => handleRegionHover('KR-48', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-48')}
                />
                <path
                  id="KR-49"
                  className="land"
                  ...
                  style={getRegionStyle('KR-49')}
                  onMouseEnter={(event) => handleRegionHover('KR-49', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-49')}
                />
                <path
                  id="KR-50"
                  className="land"
                  ...
                  style={getRegionStyle('KR-50')}
                  onMouseEnter={(event) => handleRegionHover('KR-50', event)}
                  onMouseLeave={handleRegionLeave}
                  onClick={() => handleRegionClick('KR-50')}
                />
              </g>
            </svg>
            {/* 호버 정보 표시용 오버레이 */}
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-gray-900">
                    {getRegionData(hoveredRegion)?.users.toLocaleString()}
                  </span>
                </div>
              </div>
            )}
        ...
        <div className="bg-white rounded-lg" style={{ flex: '0.80' }}>
          ...
          <div className="px-2 pb-2 space-y-2">
            {topRegions.map((region, index) => (
              <div
                key={region.id}
                className={`cursor-pointer ${selectedRegion === region.id ? 'opacity-100' : 'opacity-80 hover:opacity-100'
                  }`}
